...
if user_menu == 'Athlete-wise Analysis':
    ...
    famous_sport = [
        ...
        'Polo', 'Cricket', 'Ice Hockey']

    for sport in famous_sport:
        temp_df = athlete_df[athlete_df['Sport'] == sport]
        gold_ages = temp_df[temp_df['Medal'] == 'Gold']['Age'].dropna()

        if len(gold_ages) > 1:  # Ensure each dataset has more than one element
            x.append(gold_ages)
            name.append(sport)

    # Create distplot only with valid non-empty sequences
    if x and name:
        ...
        st.title("No sufficient data available for the selected sports")
    ...
    st.title("Men Vs Women Participation over year")
